...
app.get("/random", ( req,res ) => {
    ...
    const random = (jokes[result]);
    console.log(random);
    
    res.render("index.ejs", {
        joke: random.jokeText,
        jokeType: random.jokeType,
    })
})
app.get("/jokes/:id", (req,res) =>{
   
    const getJoke = jokes.find((j) => j.id === parseInt(req.params.id))
    if (!getJoke) return res.status(404).send( `The Joke with the given ID was not found`);
    console.log("Found Joke :", getJoke);
    res.render("index.ejs",{
        joke: getJoke.jokeText,
        jokeType: getJoke.jokeType,
    })
